...
<template>
    <div v-for="genre, index in genres" :key="index" @click="gotoPageGenre(genre.title)"  class="bg-[rgba(0,0,0, 0.7)] rounded-xl flex justify-center items-center h-52 w-80 relative transition-all hover:scale-105 ">
        <img :src="genre.img" class="h-full w-full absolute opacity-30 transition-opacity hover:opacity-100 " >
